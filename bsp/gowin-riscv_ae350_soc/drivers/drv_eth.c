...
static rt_err_t phy_write_reg(rt_uint8_t phy_addr, rt_uint8_t reg_addr, rt_uint16_t reg_value)
{
    rt_uint32_t temp;
	...
	AE350_ETHERNET->MIIM_PHY_ADDR = phy_addr;
	AE350_ETHERNET->MIIM_REG_ADDR = reg_addr;
	AE350_ETHERNET->MIIM_WR_DATA = reg_value;
	AE350_ETHERNET->MIIM_OP_EN = 1;
    /* Wait for the write to complete */
    tickstart = rt_tick_get();
    while((AE350_ETHERNET->MIIM_IS & MIIM_OP_END_IS) == 0)
    {
        /* judge timeout */
    ...
    /* Clear the interrupt */
    AE350_ETHERNET->MIIM_IC = MIIM_OP_END_IC;

    return RT_EOK;
}

static rt_err_t phy_write_paged_reg(rt_uint8_t phy_addr, rt_uint16_t page, rt_uint8_t reg_addr, rt_uint16_t reg_value)
{
...
static rt_ssize_t phy_read_reg(rt_uint8_t phy_addr, rt_uint8_t reg_addr)
{
    ...
    tickstart = rt_tick_get();
    while(((AE350_ETHERNET->MIIM_IS & MIIM_OP_END_IS) == 0) || ((AE350_ETHERNET->MIIM_IS & MIIM_RD_DATA_VALID_IS) == 0))
    {
        /* judge timeout */
        if((rt_tick_get() - tickstart) > ETH_TIME_OUT)
        {
    ...
    /* Get register value */
    reg_value = AE350_ETHERNET->MIIM_RD_DATA;

    /* Clear the interrupt */
    AE350_ETHERNET->MIIM_IC = MIIM_OP_END_IC | MIIM_RD_DATA_VALID_IC;
...
static rt_err_t update_mac_mode(rt_uint32_t eth_speed, rt_uint32_t eth_mode)
{
    ...
    {
        AE350_ETHERNET->ETH_MODE = ETH_FULL_DUPLEX_1000M;
    }
    else if ((eth_speed == PHY_100M) && (eth_mode == PHY_FULL_DUPLEX))
    {
        AE350_ETHERNET->ETH_MODE = ETH_FULL_DUPLEX_100M;
    }
    else if ((eth_speed == PHY_100M) && (eth_mode == PHY_HALF_DUPLEX))
    {
        AE350_ETHERNET->ETH_MODE = ETH_HALF_DUPLEX_100M;
    }
    else if ((eth_speed == PHY_10M) && (eth_mode == PHY_FULL_DUPLEX))
    {
        AE350_ETHERNET->ETH_MODE = ETH_FULL_DUPLEX_10M;
    }
    else if ((eth_speed == PHY_10M) && (eth_mode == PHY_HALF_DUPLEX))
    {
        AE350_ETHERNET->ETH_MODE = ETH_HALF_DUPLEX_10M;
    }
    else
    {
...
static rt_ssize_t rt_bsp_eth_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    LOG_D("emac read");
...
static rt_ssize_t rt_bsp_eth_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    LOG_D("emac write");
    rt_set_errno(-RT_ENOSYS);
    return 0;
...
rt_err_t rt_bsp_eth_tx(rt_device_t dev, struct pbuf *p)
{
	...
	AE350_ETHERNET->ETH_TX_LENGTH = framelen;
    ...
    tickstart = rt_tick_get();
    while((AE350_ETHERNET->ETH_TX_IS & 0x1) == 0)
    {
        /* judge timeout */
        if((rt_tick_get() - tickstart) > ETH_TIME_OUT)
        {
...
static void phy_linkchange()
{
    ...
    if (status & (RTL8211F_BMSR_LINK_STATUS | RTL8211F_INSR_AN_COMPLETE))
    {
        ...
        if (status & RTL8211F_BMSR_LINK_STATUS)
        {
            ...
            bsp_eth_device.eth_mode = (status & RTL8211F_PHYSR_DUPLEX)? PHY_FULL_DUPLEX : PHY_HALF_DUPLEX ;
...
static void eth_phy_isr(void *args)
{
    ...
    phy_read_reg(RTL8211F_PHY_ADDR, PHY_INTERRUPT_FLAG_REG, (rt_uint32_t *)&status);
...
static void phy_monitor_thread_entry(void *parameter)
{
    ...
    bsp_eth_device.poll_link_timer = rt_timer_create("phylnk", (void (*)(void*))phy_linkchange,
                                        NULL, RT_TICK_PER_SECOND, RT_TIMER_FLAG_PERIODIC);
    if (!bsp_eth_device.poll_link_timer || rt_timer_start(bsp_eth_device.poll_link_timer) != RT_EOK)
    {
    ...
    while(1)
    {
                      ...
                      RT_WAITING_FOREVER, &status) == RT_EOK)
        {
            ...
            if (!rxDmaDesc[rxIndex].valid)
            {
                ...
                rt_uint32_t rx_length = AE350_ETHERNET->ETH_RX_LENGTH;
                *rx_data_length = rx_length;

                rt_uint8_t *rx_buffer = (rt_uint8_t *)(AE350_ETHERNET->ETH_RX_DATA);
                ...
                /* Clear the interrupt */
                AE350_ETHERNET->ETH_RX_IC = 0x1;
                AE350_ETHERNET->ETH_RX_IE = 0x1;
...
static int rt_hw_bsp_eth_init(void)
{
    ...
    rt_hw_interrupt_install(IRQ_ETH_SOURCE, ETH1_IRQHandler, RT_NULL, "eth");
    rt_hw_interrupt_umask(IRQ_ETH_SOURCE);
